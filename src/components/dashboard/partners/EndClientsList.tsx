...
export default function EndClientsPage({ embedded = false }: { embedded?: boolean }) {
  const { endClients, isLoading, error, deleteEndClient, fetchEndClients } = useEndClients();
      ...
      <div className="surface">
        ...
        {!isLoading && rows.length > 0 && (
          <div className="border-t border-slate-100 px-5 py-3">
            <p className="text-xs text-slate-500">{rows.length} of {valid.length} end client{valid.length !== 1 ? 's' : ''}</p>
          </div>
        )}
      </div>
