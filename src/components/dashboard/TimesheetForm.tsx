    ...
    <form onSubmit={submit} className="surface space-y-5 p-5 sm:p-6">
      <div className="grid gap-4 sm:grid-cols-2">
        <div>
          ...
          <select value={employeeId} onChange={(e) => onEmployeeChange(e.target.value)} className={input}>
        ...
        <div>
          ...
          <select value={status} onChange={(e) => setStatus(e.target.value as TimesheetStatus)} className={input}>
            {TIMESHEET_STATUSES.map((s) => <option key={s} value={s}>{s}</option>)}
          </select>
        </div>
        <div>
          <label className={label}>Bill rate ($/hr)</label>
          <input type="number" min={0} step="0.01" value={billRate} onChange={(e) => setBillRate(e.target.value)} placeholder="0.00" className={input} />
        </div>
        <div>
          <label className={label}>Pay rate ($/hr)</label>
          <input type="number" min={0} step="0.01" value={payRate} onChange={(e) => setPayRate(e.target.value)} placeholder="0.00" className={input} />
        </div>
      </div>

      <div>
        <label className={label}>Notes</label>
        <textarea value={notes} onChange={(e) => setNotes(e.target.value)} rows={2} placeholder="Optional notes…" className={input} />
      </div>
      ...
      <div className="flex justify-end gap-2 border-t border-slate-100 pt-4">
        <button type="button" onClick={() => router.push('/dashboard/timesheets')} className="btn-ghost">Cancel</button>
