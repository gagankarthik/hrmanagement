...
/**
 * Own the chart measurement instead of recharts' ResponsiveContainer, whose
 * ResizeObserver misses updates under React 19 dev double-mounts and leaves
 * charts frozen at 9×9. One observer per chart wrapper, explicit pixel sizes.
 */
...
// Cohesive console palette — cobalt-first, matching --chart-1..5
export const VIZ = {
  brand: '#1d4ed8',
  blue: '#2563eb',
  violet: '#7c3aed',
...
function TooltipBox({ active, payload, label }: any) {
  ...
  return (
    <div className="rounded-[8px] border border-[var(--adm-line)] bg-white px-3 py-2 text-xs shadow-[var(--adm-shadow-pop)]">
      {label != null && label !== '' && <p className="mb-1 font-semibold text-[var(--adm-ink)]">{label}</p>}
...
export function CompareBarChart({
  ...
  return (
    <ChartBox height={height}>
      {(width) => (
        <BarChart width={width} height={height} data={data} margin={{ top: 8, right: 8, left: -18, bottom: 0 }} barGap={4}>
...
export function HBarChart({
  ...
  return (
    <ChartBox height={height}>
      {(width) => (
        <BarChart width={width} height={height} layout="vertical" data={data} margin={{ top: 4, right: 52, left: 8, bottom: 4 }} barCategoryGap={12}>
          <CartesianGrid horizontal={false} stroke="var(--adm-line)" strokeDasharray="3 3" />
...
export function TrendAreaChart({
  ...
  return (
    <ChartBox height={height}>
      {(width) => (
        <AreaChart width={width} height={height} data={data} margin={{ top: 8, right: 8, left: -18, bottom: 0 }}>
          <defs>
            {areas.map((a) => (
