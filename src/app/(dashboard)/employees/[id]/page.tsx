...
function EmployeeDetailPageContent() {
  ...
  if (!employee) {
    return (
      <EmptyState
        ...
        action={<button onClick={() => router.push('/employees')} className="btn-primary">Back to Employees</button>}
        className="mt-12"
      />
    );
  }
  ...
  // captured for this employee type, grouped by the same shared sections — so the
  // view always shows what was entered (new == edit == onboard == view).
  const detailRec = employee as unknown as Record<string, unknown>;
  const detailValue = (field: FormField): React.ReactNode => {
    const raw = detailRec[field.name];
    ...
    if (field.type === 'date') {
      const formatted = fmt(String(raw));
      return field.name === 'dob' && formatted && age != null ? `${formatted} (age ${age})` : formatted;
    }
    if (field.type === 'number') {
      const n = Number(raw);
      if (Number.isNaN(n)) return String(raw);
      return /pay|salary|rate|reimbursement/i.test(field.name) ? `$${n.toLocaleString()}` : String(n);
    }
    return String(raw);
  };
  const detailFields = getFieldsByType(employee.type).filter((f) => f.name !== 'clientId' && f.name !== 'vendorId');
  const detailSections = EMPLOYEE_FORM_SECTIONS
    .map((s) => ({ ...s, sectionFields: detailFields.filter((f) => sectionForField(f.name) === s.id) }))
    .filter((s) => s.sectionFields.length > 0);
  const otherDetailFields = detailFields.filter((f) => sectionForField(f.name) === 'other');
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="flex flex-wrap items-center justify-between gap-2">
        <button onClick={() => router.push('/employees')} className="flex items-center gap-2 rounded-xl px-3 py-2 text-sm font-medium text-slate-600 transition-all hover:bg-white hover:shadow-sm">
        ...
        <div className="flex flex-wrap items-center gap-2.5">
          ...
          <ActionMenu items={[
            { label: 'Edit', icon: Pencil, onClick: () => router.push(`/employees/${employeeId}/edit`) },
          ...
          <div className="p-5 sm:p-6">
              ...
              <div className="space-y-6">
                {detailSections.map((section) => (
                  <DetailSection key={section.id} title={section.title}>
                    {section.sectionFields.map((f) => (
                      <Field key={f.name} label={f.label} value={detailValue(f)} />
                    ))}
                ...
                {otherDetailFields.length > 0 && (
                  <DetailSection title="Additional Details">
                    {otherDetailFields.map((f) => (
                      <Field key={f.name} label={f.label} value={detailValue(f)} />
                    ))}
                  </DetailSection>
                )}
                  ...
                  {leaveBalance && (
                    <div className="rounded-2xl border border-slate-100 bg-slate-50/50 p-5">
                      ...
                      {leaveBalance.allowance === 0 ? (
                        <p className="text-sm text-slate-500">No leave policy set. <Link href="/policies" className="font-semibold text-brand-600">Configure →</Link></p>
                      ) : (
                        <>
            ...
            {tab === 'workauth' && (
              <div className="space-y-6">
                ...
                <Link href={`/i9/${employeeId}`} className="inline-flex items-center gap-1.5 text-sm font-semibold text-brand-700 hover:underline">Manage work authorization & I-9 <ChevronRight className="h-4 w-4" /></Link>
              </div>
            )}
            ...
            {tab === 'forms' && (
              <div className="grid gap-3 sm:grid-cols-2">
                <Link href={`/i9/${employeeId}`} className={tileBase}>
                ...
                <Link href={`/i983/${employeeId}`} className={tileBase}>
                ...
                <Link href={`/documents/${employeeId}`} className={tileBase}>
              ...
              <div className="space-y-4">
                <div className="flex flex-col gap-3 rounded-2xl border border-slate-100 bg-slate-50/50 p-5 sm:flex-row sm:items-center sm:justify-between">
                  ...
                  <div className="flex flex-wrap gap-2">
                    ...
                    <Link href={`/i9/${employeeId}`} className="btn-primary">{i9Record ? 'Manage I-9' : 'Start I-9'}</Link>
                  </div>
                </div>
...
function FormsMenu({ employeeId }: { employeeId: string }) {
  ...
  const forms = [
    { label: 'Form I-9', icon: BadgeCheck, href: `/i9/${employeeId}` },
    { label: 'Form I-983', icon: GraduationCap, href: `/i983/${employeeId}` },
  ];
