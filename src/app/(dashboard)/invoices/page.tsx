        ...
        {valid.length === 0 ? (
          <div className="p-5">
            <EmptyState
              ...
              action={<button onClick={() => router.push('/invoices/new')} className="btn-primary"><Plus className="h-4 w-4" strokeWidth={1.75} /> New invoice</button>}
            />
          </div>
        ) : (
                  ...
                  >
                    ...
                    <td className="px-5 py-3.5" onClick={(e) => e.stopPropagation()}>
                      <div className="flex items-center justify-end gap-1">
                        <ActionMenu
                          items={[
                            { label: 'View', icon: Eye, onClick: () => router.push(`/invoices/${inv.id}`) },
