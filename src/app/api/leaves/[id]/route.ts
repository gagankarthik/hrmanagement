...
export async function GET(
  ...
  try {
    ...
    return NextResponse.json(
      { success: false, error: err.message || 'Failed to fetch leave' },
      { status: 500 }
    );
  }
}
...
export async function PUT(
  ...
  try {
    ...
    const item = {
      ...body,
      id,
      days: computeDays(body.startDate, body.endDate),
      PK: `LEAVE#${id}`,
      SK: `LEAVE#${id}`,
      GSI1PK: 'LEAVES',
      GSI1SK: body.GSI1SK || `LEAVE#${body.createdAt || now}`,
      updatedAt: now,
    };
