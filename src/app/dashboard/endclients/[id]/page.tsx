...
function EndClientDetailPageContent() {
  ...
  const endClientId = Array.isArray(params.id) ? params.id[0] : (params.id ?? '');
  ...
  const endClient = useMemo(() => {
    if (!endClientId) return undefined;
    return endClients.find((c) => c.id === endClientId);
  }, [endClients, endClientId]);

  const endClientEmployees = useMemo(() => {
    if (!endClient) return [];
    return employees.filter((emp) => {
      const inAssignments = emp.endClientAssignments?.some((a) => a.clientId === endClient.id);
      return inAssignments || emp.endClientId === endClient.id;
    });
  }, [employees, endClient]);

  const activeCount = useMemo(
    () => endClientEmployees.filter((e) => 'status' in e && (e as { status: string }).status === 'Active').length,
    [endClientEmployees]
  );

  const typeDistribution = useMemo(() => {
    const dist: Record<string, number> = {};
    endClientEmployees.forEach((e) => { dist[e.type] = (dist[e.type] || 0) + 1; });
    return dist;
  }, [endClientEmployees]);
  ...
  const handlePrint = () => {
    if (!endClient) return;
    const rows = endClientEmployees.map((emp) => {
    ...
    if (!win) return;
    win.document.write(`<!DOCTYPE html><html><head><title>${endClient.name} — End Client Report</title>
    <style>
    ...
    <div class="header">
      <div>
        <h1>${endClient.name}</h1>
        <p>${endClient.contactPerson ? `Contact: ${endClient.contactPerson}` : ''}${endClient.email ? ` · ${endClient.email}` : ''}${endClient.phone ? ` · ${endClient.phone}` : ''}</p>
        <p style="margin-top:6px;"><span class="badge ${endClient.status === 'Active' ? 'badge-active' : 'badge-inactive'}">${endClient.status}</span></p>
      </div>
      <div style="text-align:right;color:#64748b;font-size:12px;">
        <strong>End Client Report</strong><br/>
        Generated: ${format(new Date(), 'MMMM d, yyyy')}<br/>
        ${endClient.address ? endClient.address : ''}
      </div>
    </div>
    <div class="stats">
      <div class="stat-card"><div class="num">${endClientEmployees.length}</div><div class="lbl">Total Employees</div></div>
  ...
  if (!endClient) {
    return (
      <EmptyState
        ...
        action={
          <Link href="/dashboard/endclients" className="btn-primary">
            Back to End Clients
          </Link>
        }
        className="mt-12"
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between gap-3">
        ...
        <div className="flex items-center gap-2">
          ...
          <Link href={`/dashboard/endclients/${endClient.id}/edit`} className="btn-primary">
          ...
          <div className="flex-1 min-w-0">
            ...
            {endClient.address && (
              <p className="mt-0.5 flex items-center gap-1 text-white/80 truncate">
                <MapPin className="h-3.5 w-3.5" />{endClient.address}
              </p>
            )}
            <div className="mt-3 flex flex-wrap gap-2">
              <span className={cn(
                'inline-flex items-center gap-1 rounded-full px-2.5 py-1 text-xs font-semibold',
                endClient.status === 'Active' ? 'bg-emerald-400/30 text-white' : 'bg-red-400/30 text-white'
              )}>
                {endClient.status === 'Active' ? <CheckCircle2 className="h-3 w-3" /> : <XCircle className="h-3 w-3" />}
                {endClient.status}
              </span>
              <span className="inline-flex items-center gap-1 rounded-full bg-white/20 px-2.5 py-1 text-xs font-semibold text-white">
                <Users className="h-3 w-3" />{endClientEmployees.length} Employees
              </span>
            </div>
          </div>
        </div>
      ...
      <div className="grid grid-cols-2 gap-4 sm:grid-cols-4">
        {[
          { label: 'Total', value: endClientEmployees.length, bg: 'bg-blue-100', color: 'text-blue-600', icon: Users },
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        ...
        <div className="surface p-6">
          <div className="mb-4 flex items-center gap-2">
            ...
            <h2 className="font-display text-base font-bold text-slate-900">End Client Information</h2>
          </div>
          <div className="space-y-4">
            {endClient.contactPerson && (
              <div className="flex items-start gap-3">
                ...
                <div>
                  ...
                  <p className="mt-0.5 text-sm font-semibold text-slate-900">{endClient.contactPerson}</p>
                </div>
              </div>
            )}
            {endClient.email && (
              <div className="flex items-start gap-3">
                ...
                <div>
                  ...
                  <p className="mt-0.5 text-sm font-semibold text-slate-900">{endClient.email}</p>
                </div>
              </div>
            )}
            {endClient.phone && (
              <div className="flex items-start gap-3">
                ...
                <div>
                  ...
                  <p className="mt-0.5 text-sm font-semibold text-slate-900">{endClient.phone}</p>
                </div>
              </div>
            )}
            {endClient.address && (
              <div className="flex items-start gap-3">
                ...
                <div>
                  ...
                  <p className="mt-0.5 text-sm font-semibold text-slate-900">{endClient.address}</p>
                </div>
              </div>
            )}
            {!endClient.contactPerson && !endClient.email && !endClient.phone && !endClient.address && (
        ...
        <div className="surface p-6">
                  ...
                  <div className="flex items-center gap-3">
                    <div className="h-2 rounded-full bg-slate-100 w-28">
                      <div
                        ...
                        style={{ width: `${(count / endClientEmployees.length) * 100}%` }}
                      />
                    </div>
          ...
          <div className="mt-4 border-t border-slate-100 pt-3 text-xs text-slate-400">
            {format(new Date(endClient.createdAt), 'MMM d, yyyy')} — {format(new Date(endClient.updatedAt), 'MMM d, yyyy')}
          </div>
        </div>
      </div>
      ...
      <div className="surface">
        <div className="flex items-center justify-between border-b border-slate-100 px-6 py-4">
          <div className="flex items-center gap-2">
            ...
            <span className="rounded-full bg-slate-100 px-2 py-0.5 text-xs font-medium text-slate-500">{endClientEmployees.length}</span>
          </div>
          <button
        ...
        {endClientEmployees.length === 0 ? (
          <div className="p-5">
            <EmptyState
              ...
              description="When you assign employees to this end client they'll appear here."
              action={
          ...
          <div className="min-w-[720px] divide-y divide-slate-50">
            ...
            {endClientEmployees.map((emp, idx) => {
        ...
        <div className="border-t border-slate-100 px-5 py-3">
          <p className="text-xs text-slate-400">Showing {endClientEmployees.length} employees</p>
        </div>
      </div>
      ...
      <ConfirmDialog
        ...
        description={
          <>
            ...
            <span className="font-semibold text-slate-900">{endClient.name}</span>? This action cannot be undone.
          </>
        }
      ...
      <AssignEmployeesModal
        ...
        partnerKind="endclients"
        partnerId={endClient.id}
        partnerName={endClient.name}
      />
    </div>
  );
}
