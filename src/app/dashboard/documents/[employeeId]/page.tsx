...
export default function EmployeeDocsManagePage({ params }: { params: Promise<{ employeeId: string }> }) {
  ...
  if (!emp && !isLoading) {
    return (
      <div className="mx-auto max-w-3xl space-y-4">
        <button onClick={() => router.push('/dashboard/documents')} className="inline-flex items-center gap-1.5 text-sm font-medium text-slate-500 hover:text-brand-700"><ArrowLeft className="h-4 w-4" /> Back to Employee Documents</button>
  ...
  return (
    <div className="space-y-5">
      <button onClick={() => router.push('/dashboard/documents')} className="inline-flex items-center gap-1.5 text-sm font-medium text-slate-500 transition-colors hover:text-brand-700">
      ...
      <div className="flex justify-end gap-2">
        <button onClick={() => router.push('/dashboard/documents')} className="btn-ghost">Cancel</button>
